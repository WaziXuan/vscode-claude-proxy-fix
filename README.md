...
5. **Optional — Pin to Taskbar:**
   > Windows does not allow pinning `.vbs` shortcuts to the Taskbar directly. Use this workaround:
   - Right-click the `.vbs` file → **Create shortcut**
   - Right-click the shortcut → **Properties**
   - Change the **Target** field to:
     ```
     wscript.exe "D:\Sandbox\vscode-claude-proxy-fix\launch_vscode_proxy.vbs"
     ```
     *(adjust the path to wherever you saved the `.vbs` file)*
   - Optionally, click **Change Icon** and point it to `D:\Microsoft VS Code\Code.exe` for the VS Code icon
...
5. **可选 — 固定到任务栏：**
   > Windows 不允许直接将 `.vbs` 快捷方式固定到任务栏，需要以下步骤：
   - 右键 `.vbs` 文件 → **创建快捷方式**
   - 右键快捷方式 → **属性**
   - 将「目标」改为：
     ```
     ...
     ```
     *(路径改为你实际保存 `.vbs` 文件的位置)*
   - 可选：点击「更改图标」，指向 `D:\Microsoft VS Code\Code.exe`，使用 VS Code 图标
   - 右键快捷方式 → **固定到任务栏**
